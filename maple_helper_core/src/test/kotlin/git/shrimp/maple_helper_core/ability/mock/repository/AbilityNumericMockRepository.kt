...
import git.shrimp.maple_helper_core.ability.mock.entity.AbilityNumericMockEntity
import git.shrimp.maple_helper_core.global.types.OptionLevel

class AbilityNumericMockRepository(
    private val abilityOptionMockRepository: AbilityOptionMockRepository
) {
    private val tableByOption = mutableMapOf<Int, List<AbilityNumericMockEntity>>()
    ...
    private fun add(
        abilityNumeric: AbilityNumericMockEntity
    ) {
    ...
    private fun add(
        ...
        numerics.zip(weights).forEach { (numeric, weight) ->
            this.add(AbilityNumericMockEntity(level, weight, numeric, option))
        }
    }
    ...
    fun findAllByOptionIdAndLevel(
        optionId: Int,
        level: OptionLevel
    ): List<AbilityNumericMockEntity>? {
        return this.tableByOption[optionId]?.filter { it.level == level }
    }
}
