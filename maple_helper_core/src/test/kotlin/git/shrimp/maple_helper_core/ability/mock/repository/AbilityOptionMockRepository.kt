...
import git.shrimp.maple_helper_core.ability.mock.entity.AbilityOptionMockEntity
import java.util.*

class AbilityOptionMockRepository {
    private val table = mutableMapOf<Int, AbilityOptionMockEntity>()
    ...
    private fun add(
        ...
        val option = AbilityOptionMockEntity(id, name)
        this.table[id] = option
    }

    fun findById(
        id: Int
    ): Optional<AbilityOptionMockEntity> {
        return Optional.ofNullable(this.table[id])
    }

    fun findAll(): List<AbilityOptionMockEntity> {
