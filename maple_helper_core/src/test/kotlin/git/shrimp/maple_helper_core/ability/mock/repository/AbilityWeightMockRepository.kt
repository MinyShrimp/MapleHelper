package git.shrimp.maple_helper_core.ability.mock.repository

import git.shrimp.maple_helper_core.ability.mock.entity.AbilityWeightMockEntity
import git.shrimp.maple_helper_core.global.types.OptionLevel

class AbilityWeightMockRepository(
    private val abilityOptionMockRepository: AbilityOptionMockRepository
) {
    private val tableByRare = mutableMapOf<OptionLevel, List<AbilityWeightMockEntity>>()
    ...
    private fun add(
        abilityWeight: AbilityWeightMockEntity
    ) {
    ...
    private fun add(
        ...
        val option = this.abilityOptionMockRepository.findById(id).orElseThrow()
        this.add(AbilityWeightMockEntity(level, weight, option))
    }

    fun findByOptionIdAndLevel(
        id: Int,
        level: OptionLevel
    ): AbilityWeightMockEntity? {
        return this.tableByRare[level]!!.find { it.optionId == id }
    }
}
